...
async function initLoginWidget() {
    ...
    widget.innerHTML = `
      ...
      <div class="lw-panel" id="lwPanel">
        <div class="lw-panel-header">
          <img src="${user.user_metadata.avatar_url}" alt="avatar" class="lw-panel-img">
          <div>
            <div class="lw-name">${user.user_metadata.full_name}</div>
  ...
  // Events
  if (user) {
    document.getElementById('lwLogout').onclick = async () => {
      await signOut()
      window.location.reload()
    }
  } else {
    document.getElementById('lwLoginBtn').onclick = signInWithGoogle
  }
}
...
supabase.auth.onAuthStateChange(() => initLoginWidget())